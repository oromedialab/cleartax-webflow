...
│  src/styles/theme.css            ← @theme bindings + container      │
└────────────────────────────────────────┬────────────────────────────┘
                                         │
                       npm run build     │
                                         ▼
...
`public/css/fonts.css` is **never** pasted into Webflow. It self-hosts the **brand v1** faces (Nohemi/Gilroy) and exists only so v1 pages render in `npm run dev` and `dist/preview/*.html`.

Brand v2 pages do not import it. They load Inter Tight + Google Sans from Google Fonts with a `<link>` in the page head — the same tag goes into the Webflow page head. See [docs/brand/typography.md](./brand/typography.md).

## File-by-file walkthrough

### `src/styles/shared.css`

Small universal bundle (~10 KB). Contains:
- `@import './tokens.css'` — design-token CSS variables in `:root`.
- Element reset (`*, body, h1…h6, a, img`) at **top level** (unlayered).
- Custom helpers (`.tw-block`, `.tw-grid`, `.nums-tabular`, `.font-inter-tight`, `.bg-grad-recon`, animation classes) at top level (unlayered).
- The keyboard focus ring (`:focus-visible`) and the `prefers-reduced-motion` block.
...
`scripts/build-css.mjs` builds `TARGETS` = `shared` + `fonts` + the `PAGE_TARGETS` array imported from [scripts/css-entries.mjs](../scripts/css-entries.mjs) (shared with the dev-server plugin in `astro.config.mjs` so both see one list). A page entry looks like:
...
4. No CSS file edit, no `PAGE_TARGETS` edit. **Never** hand-add an `@source` for it — and never the `_shared/**/*.astro` wildcard, which compiles all 12 shared components into the page (5–12 KB wasted against the 50 KB cap).

### Add an entirely new page

See [docs/adding-a-new-page.md](./adding-a-new-page.md). The short version: create page astro, sections folder, `<page>.css` and append a `PAGE_TARGETS` entry in `scripts/css-entries.mjs`.
...
| `http://localhost:4321/embed-build/<page>/<section>` | one section's raw markup, **unstyled** — this route exists to generate embeds, not to preview them |
